...
class VisionEngine:
    ...
    def __init__(self, cfg: dict, calib: dict,
                 sensor_res: tuple, low_res: tuple):
        ...
        # als de res < full sensor (2304×1296) (bvb bij 120fps sensor crop), dan moet er een offset zijn
        _full = (2304, 1296)
        ...
        # Vorige detecties
        # Omdat de cameras geen gesynchroniseerde shutter hebben, kunnen de left en right frame een andere tijd hebben
        # --> geeft een fout offset in de triangulatie
        # --> door history te interpoleren, kan 1 object wel een L en R coordinaat hebben op dezelfde timestamp.
        self._history = {"l": [], "r": []}

    def set_tracking_color(self, center_hsv: np.ndarray):
        self._center_hsv = np.array(center_hsv, dtype=np.uint8)
        self._update_hsv_bounds()

    def set_tracking_color_margins(self, h_margin: int, sv_margin: int):
        self._h_margin  = h_margin
        self._sv_margin = sv_margin
        self._update_hsv_bounds()

    def get_tracking_color_state(self) -> dict:
        return {
    ...
    # leuk voor UI want je kan gwn als eyedropper tool klikken op een gekleurd object.
    ...
    def process(self, img_l: np.ndarray, t_l: float,
        ...
        for side, img, t in (("l", img_l, t_l), ("r", img_r, t_r)):
            result = self._find_colored_object(img, t, side)
            targets[side] = result
        ...
        # Annotate debug visuals
        out_l = self._annotate_debug_visuals(img_l.copy(), targets["l"])
        out_r = self._annotate_debug_visuals(img_r.copy(), targets["r"])

        return {
            "img_l":    out_l,          # Output frames hebben debug visuals
            "img_r":    out_r,
            "found":    found,
            "pos":      pos,            # 3D positie (triangulated)
            "target_l": targets["l"],   # 2D positie L
            "target_r": targets["r"],   # 2D positie R
        }
    ...
    def _find_colored_object(self, img: np.ndarray, t: float, side: str):
        ...
        # Als er geen object getracked is, dan is de history ook ongeldig voor de toekomst
        self._history[side] = []
        return None

    def _temporal_interpolate(self, side: str, target_time: float):
    ...
    def _lo_res_coord_to_full_sensor(self, x: float, y: float) -> tuple:
        """Scale stream coords → full-sensor coords (with crop offset)."""
        sx = self._sensor_res[0] / self._low_res[0]
        sy = self._sensor_res[1] / self._low_res[1]
        return x * sx + self._offset_x, y * sy + self._offset_y

    # rectification moet in full res coords gebeuren
    ...
    # triangulatie moet in full res coords gebeuren
    # output: in mm
    def _triangulate(self, tgt_l: tuple, tgt_r: tuple, t_r: float):
        # Interpolate left detection to match right camera timestamp
        interp_l = self._temporal_interpolate("l", t_r)
        if interp_l is None:
            interp_l = (tgt_l[0], tgt_l[1])

        raw_xl, raw_yl = self._lo_res_coord_to_full_sensor(*interp_l)
        raw_xr, raw_yr = self._lo_res_coord_to_full_sensor(tgt_r[0], tgt_r[1])
    ...
    @staticmethod
    def _annotate_debug_visuals(img: np.ndarray, target) -> np.ndarray:
